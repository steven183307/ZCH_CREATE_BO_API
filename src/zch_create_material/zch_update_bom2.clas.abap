CLASS zch_update_bom2 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCH_UPDATE_BOM2 IMPLEMENTATION.
...
*    UPDATE FROM update_bom2
   ...
   REPORTED DATA(reported).
    ...
    IF Failed IS NOT INITIAL.
      LOOP AT reported-billofmaterial ASSIGNING FIELD-SYMBOL(<lt_proudct1>).
        DATA(lv_result1) = <lt_proudct1>-%msg->if_message~get_text( ).
        out->write( 'BOM1 error' ).
        out->write( lv_result1 ).
      ENDLOOP.
      LOOP AT reported-billofmaterialitem ASSIGNING FIELD-SYMBOL(<lt_proudct3>).
        DATA(lv_result3) = <lt_proudct3>-%msg->if_message~get_text( ).
        out->write( 'BOM3 error' ).
    ...
    ELSE.
      ...
      IF failed_commit IS NOT INITIAL.
        LOOP AT reported_commit-billofmaterial ASSIGNING FIELD-SYMBOL(<lt_proudct2>).
          DATA(lv_result2) = <lt_proudct2>-%msg->if_message~get_text( ).
          out->write( 'BOM2 error' ).
          out->write( lv_result2 ).
        ENDLOOP.
        LOOP AT reported-billofmaterialitem ASSIGNING FIELD-SYMBOL(<lt_proudct4>).
        DATA(lv_result4) = <lt_proudct4>-%msg->if_message~get_text( ).
        out->write( 'BOM4 error' ).
